...
func getMinCost(cost []int) int {
	...
	var min1, min2 int
	min1 = cost[0] + getMinCost(cost[1:])
	min2 = cost[0] + getMinCost(cost[2:])
	if min1 < min2 {
		return min1
	}
	return min2
...
func minCostClimbingStairs0(cost []int) int {
	...
	min1 = minCost(cost)
	if len(cost[1:]) == 3 {
		if cost[1]+cost[3] < cost[1]+cost[2] {
			min2 = cost[1] + cost[3]
		} else {
			min2 = cost[1] + cost[2]
		}
...
func minCost(cost []int) int {
	...
	if cost[len(cost)-1]+cost[len(cost)-3] > cost[len(cost)-2] {
		mincost = cost[len(cost)-2] + minCost(cost[0:len(cost)-2])
	} else {
		mincost = cost[len(cost)-1] + cost[len(cost)-3] + minCost(cost[0:len(cost)-3])
	}
	return mincost
}
